...
# audio_processor.py - FINALIZED FIX
...
try:
    logger.info(f"Initializing faster-whisper model: {MODEL_SIZE} on {DEVICE} ({COMPUTE_TYPE})")
    # FIX/ENHANCEMENT: Try loading locally first for speed and stability
    WHISPER_MODEL = WhisperModel(MODEL_SIZE, device=DEVICE, compute_type=COMPUTE_TYPE, local_files_only=True)
    logger.info("✅ Faster-Whisper model loaded locally.")
except Exception as e:
    # FALLBACK: Try network download if local file loading failed
    try:
        logger.warning(f"Failed to load model locally: {e}. Attempting network download.")
        WHISPER_MODEL = WhisperModel(MODEL_SIZE, device=DEVICE, compute_type=COMPUTE_TYPE, local_files_only=False)
        logger.info("✅ Faster-Whisper model loaded via network.")
    except Exception as e_fallback:
        logger.error(f"❌ Failed to load Faster-Whisper model: {e_fallback}. Audio processing disabled.")
        WHISPER_MODEL = None
